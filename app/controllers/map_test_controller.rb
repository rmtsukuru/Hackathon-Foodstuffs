class MapTestController < ApplicationController
  ...
  def parse_query
    ...
    if categories.empty?
      ...
      if (radius.class == Fixnum) and terms.empty?
    ...
    path = "/v2/search?term=#{URI::encode terms.join(' ')}&ll=#{params[:latitude]},#{params[:longitude]}&radius_filter=#{radius.to_i}&limit=#{limit}&category_filter=#{categories.join(',')}"
